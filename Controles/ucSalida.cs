...
namespace GestionDeStock.Controles
{
    public partial class ucSalida : UserControl
    {
        ...
        private void Iniciar()
        {
            ...
            foreach (DataGridViewColumn col in grillaSalida.Columns)
            {
        ...
        private void consultar(string filtro)
        {
            ...
            using (var context = new StockBDContext())
            {
                var salidas = context.Movimientos
                    .Where(i => i.Tipo == Models.TipoMovimiento.Salida)
                    .Select(i => new
                    {
                        i.Fecha,
                ...
                if (categoriaId != 0)
                {
                    salidas = salidas.Where(i => i.Categoria.CategoriaId == categoriaId).ToList();
                    if (subcategoriaId != 0)
                    {
                        salidas = salidas.Where(i => i.Subcategoria.Id == subcategoriaId).ToList();
                    }
                }
                if (depositoId != 0)
                {
                    salidas = salidas.Where(i => i.Deposito != null && i.Deposito.Id == depositoId).ToList();
                }
                if (!string.IsNullOrEmpty(filtro))
                {
                    salidas = salidas.Where(i => i.Descripcion.ToLower().Contains(filtro.ToLower()) || i.CodigoArticulo.ToString().Contains(filtro)).ToList();
                }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            var editarSalida = new EditarMovimiento(TipoMovimiento.Salida);
            ...
            menu.Items.Add("Salidas", null, (s, ev) => { editarSalida.ShowDialog(); consultar(""); });
        ...
        private void grillaSalida_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0) return;

            if (grillaSalida.Columns[e.ColumnIndex].Name == "Descripción" || grillaSalida.Columns[e.ColumnIndex].Name == "Id")
            {
                var fila = grillaSalida.Rows[e.RowIndex];
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            using (var context = new StockBDContext())
            {
                ...
                var eliminarSalida = new EliminarSeleccionar(TipoDeDatos.Movimientos, TipoMovimiento.Salida);
                var editarSubcategoria = new EditarSubcategoria();
                ContextMenuStrip menu = new ContextMenuStrip();
                menu.Font = new Font(menu.Font.FontFamily, 11);
                menu.Items.Add("Salida", null, (s, ev) => { eliminarSalida.ShowDialog(); consultar(""); });
